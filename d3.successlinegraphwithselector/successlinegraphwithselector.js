/**
 * @file
 * D3 Success Line Graph library js file.
 */

(function($) {

  Drupal.d3.successlinegraphwithselector = function (select, settings) {
    var id = settings.id;
    var rows = settings.rows;
    var key = settings.legend || [];
    var datacols = settings.datacolumns || ['success', 'failure', 'total'];
    var fullwidth = 1075;
    var fullheight = 380;
    var clipid = id + '-clip';
  ...
  var successcircles = focus.append("g")
    .attr('class', 'circle-group-success line-point');
   
  var successpoint = successcircles.selectAll('g')
    .data(data)
    .enter().append('g');
  
  // outer circle
  successpoint.append('circle')
    ...
    .attr("class", function(d,i) { return 'circle-outer circle-' + i + '-outer path_success'; })
  ...
  successpoint.append('circle')
    ...
    .attr("r", circleradius)
    .style("fill", "white")
    .attr("class", function(d,i) { return 'circle circle-' + i + '-outer path_success'; })
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  successpoint.append('clipPath').attr("id", function (d, i) { 
      return "clip-success-" + i;
    }).append('circle')
  ...
  var failurecircles = focus.append("g")
    .attr('class', 'circle-group-failure line-point');
  var failurepoint = failurecircles.selectAll('g')
    .data(data)
    .enter().append('g');
  // outer circle
  failurepoint.append('circle')
    .attr('clip-path', 'url(#'+clipid+'-circle'+')')
    .attr("cx", function(d) { return x(d.datetime) })
    ...
    .attr("field", 'failure')
    .attr('fill-opacity', 0.2)
    .attr("class", function(d,i) { return 'circle-outer circle-' + i + '-outer path_failure'; })
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  // inner circle
  failurepoint.append('circle')
    ...
    .attr("r", circleradius)
    .style("fill", "white")
    .attr("class", function(d,i) { return 'circle circle-' + i + '-outer path_failure'; })
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  failurepoint.append('clipPath').attr("id", function (d, i) { return "clip-failure-" + i;}).append('circle')
    .attr("class", function(d,i) { return 'circle-mouse circle-' + i + '-mouse'; })
    .attr("fill", "#000000")
    .attr("color", "#000000")
    .attr("cx", function(d,i) { return x(d.datetime); })
    .attr("cy", function(d,i) { return y(d.failure); })
    .attr("r", sensitiveradius)
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  
  var totalcircles = focus.append("g")
    .attr('class', 'circle-group-total line-point');
  var totalpoint = totalcircles.selectAll('g')
    .data(data)
    .enter().append('g');
  //outer circle
  totalpoint.append('circle')
    .attr('clip-path', 'url(#'+clipid+'-circle'+')')
    .attr("cx", function(d) { return x(d.datetime) })
    ...
    .attr("field", 'total')
    .attr('fill-opacity', 0.2)
    .attr("class", function(d,i) { return 'circle-outer circle-' + i + '-outer path_total'; })
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  // inner circle
  totalpoint.append('circle')
    ...
    .attr("r", circleradius)
    .style("fill", "white")
    .attr("class", function(d,i) { return 'circle circle-' + i + '-outer path_total'; })
    .on("mouseover", mouseover)
    .on("mouseout", mouseout);
  totalpoint.append('clipPath').attr("id", function (d, i) { return "clip-total-" + i;}).append('circle')
    .attr("class", function(d,i) { return 'circle-mouse circle-' + i + '-mouse'; })
    .attr("fill", "#000000")
    .attr("color", "#000000")
    .attr("cx", function(d,i) { return x(d.datetime); })
